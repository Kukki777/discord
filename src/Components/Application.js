...
function Application(props) {
  const { window, uid, fallbackUserDetails } = props;
  const classes = useStyles();
  const theme = useTheme();
  const [mobileOpen, setMobileOpen] = React.useState(false);
  const [anchorEl, setAnchorEl] = React.useState(null);
  const [userDetails, setUserDetails] = useState(fallbackUserDetails);
  const [editProfileModal, setEditProfileModal] = useState(false);
  const [alert, setAlert] = useState(false);
  const open = Boolean(anchorEl);

  useEffect(() => {
    if (fallbackUserDetails) {
      setUserDetails((currentDetails) => currentDetails || fallbackUserDetails);
    }
  }, [fallbackUserDetails]);
  ...
  useEffect(() => {
    db.collection("users")
      ...
      .onSnapshot((doc) => {
        ...
        console.error("Failed to load user details from Firestore:", error);
        setUserDetails(fallbackUserDetails);
      });
  }, [uid, fallbackUserDetails]);
