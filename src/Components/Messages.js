...
import { getStoredUserDetails } from "../utils/userDetails";
...
function Messages({ values, msgId }) {
  ...
  const userData = getStoredUserDetails();
  ...
  const userLiked = uid ? values.likes?.[uid] : false;
  const userFire = uid ? values.fire?.[uid] : false;
  const userHeart = uid ? values.heart?.[uid] : false;
