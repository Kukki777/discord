...
function Rooms() {
  ...
  useEffect(() => {
    db.collection("channels")
      ...
      .onSnapshot((snapshot) => {
        ...
        console.error("Failed to load channel list from Firestore:", error);
        setAlertMessage("Firestore Database enable/rules check karo");
        setAlert(true);
      });
  }, []);
  ...
  const addChannel = (cName) => {
    if (cName) {
      ...
      if (cName === "") {
        showAlert("Room name empty nahi ho sakta");
        return;
      }

      for (var i = 0; i < channelList.length; i++) {
        if (cName === channelList[i].channelName) {
          showAlert("Room Name Already Exists!!");
          return;
        }
      }
      ...
      db.collection("channels")
        ...
        .catch((err) => {
          ...
          showAlert("Channel create nahi hua. Firestore rules check karo.");
        });
    }
  };
